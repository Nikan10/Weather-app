...
const APIkey = '0125e097ecb000d50716a578dad1dd36';

const App = () => {
  const [data, setData] = useState(null);
  const [location, setLocation] = useState('Kabul');
  ...
  switch(data.weather[0].main) {
    ...
    case 'Rain':
      ...
      break;
    case 'Clear':
      icon = <IoMdSunny className='bg-yellow'/>
      break;
    case 'Drizzle':
  ...
  return (
    <div className='w-full h-screen bg-gradientBg bg-no-repeat bg-cover bg-center flex flex-col items-center justify-center px-4 lg:px-0'>
      ...
      <form className={`${animate ? 'animate-shake' : 'animate-none'} h-14 bg-black/30 w-full max-w-[450px] rounded-full backdrop-blur-[32px] mb-4`}>
        <div className='h-full relative flex items-center justify-between p-2'>
          ...
          <button onClick={(e) => handleSubmit(e)} className='bg-[#1ab8ed] hover:bg-[#15abdd] w-16 h-10 rounded-full flex justify-center items-center transition'><IoMdSearch className='text-2xl text-white' /></button>
        </div>
      </form>
      <div className='w-full max-w-[450px] bg-black/20 min-h-[500px] min-w-[350px] text-white backdrop-blure-[32px] rounded-[32px] py-12 px-6'>
